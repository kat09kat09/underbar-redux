(function() {
  ...
  _.uniq = function(array) {
    ...
    for(var i = 0; i < array.length; i++){
      if(_.indexOf(resultArray, array[i]) === -1){
    ...
    if(Array.isArray(collection)) {
      ...
      for (var key in collection) {
        if(collection.hasOwnProperty(key)) {
          ...
          result = iterator(collection[key], key, collection);
        }
      }
    }
    return result;
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function (current, next){
      return current && Boolean(iterator(next));
    }, true);
  };
  ...
  _.extend = function(obj) {
    ...
    for(var i=0; i< args.length; i++) {
      //iterate on args element, add key & value onto obj
  ...
  _.memoize = function(func) {
    ...
    var oldArgStr;
    return function () {
      if(oldArgStr !== JSON.stringify(arguments) || results === undefined) {
        oldArgStr = JSON.stringify(arguments);
        results = func.apply(null, arguments);
      }
      return results;
    };
  };
